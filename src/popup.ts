...
inputEl.addEventListener('keydown', (e) => {
  ...
  const gridActive = currentMode() === 'emoji'
...
async function executeItem(item: RemoteItem, altAction: boolean): Promise<void> {
  ...
  if (item.kind === 'tab') {
    ...
    await chrome.runtime.sendMessage({ type: 'restore-session', sessionId: item.sessionId })
  } else if (item.commandId === 'copy-page-url' || item.commandId === 'copy-page-md') {
    const tab = SRC_TAB
      ? await chrome.tabs.get(SRC_TAB).catch(() => undefined)
      : (await chrome.tabs.query({ active: true, currentWindow: true }))[0]
    if (tab?.url) {
      await copyText(
        item.commandId === 'copy-page-md' ? `[${tab.title ?? tab.url}](${tab.url})` : tab.url,
      )
    }
    closeSelf()
    return
